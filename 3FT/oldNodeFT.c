...
/*
  Links new child oNChild into oNParent's children file/directory array
 (based on isDirec) at index ulIndex. 
...
static int Node_addChild(Node_T oNParent, Node_T oNChild,
                         size_t ulIndex, boolean isDirec)
{
...
*/
int Node_new(boolean isDirec, Path_T oPPath, Node_T oNParent,
             Node_T *poNResult, void *pvContents, size_t ulLength)
{
   ...
   if(oNParent != NULL) {
      ...
      if(Node_hasChild(oNParent, oPPath, &ulIndex, &isDirec)) {
         Path_free(psNew->oPPath);
         free(psNew);
         *poNResult = NULL;
   ...
   if(oNNode->oNParent != NULL) {
      ...
      if(oNNode->oNParent->oDFileChildren != NULL &&
         ...
         while(DynArray_getLength(oNNode->oDDirChildren) != 0) {
            ulCount += Node_free(DynArray_get(oNNode->oDDirChildren,
                                              0));
         }
         ...
         while(DynArray_getLength(oNNode->oDFileChildren) != 0){ 
            ...
            ulCount += Node_free(DynArray_get(oNNode->oDFileChildren,
                                              0));
         }
...
boolean Node_hasChild(Node_T oNParent, Path_T oPPath,
   ...
   i =  DynArray_bsearch(oNParent->oDDirChildren,
   ...
   j =  DynArray_bsearch(oNParent->oDFileChildren,
                         (char*) Path_getPathname(oPPath), pulChildID,
                         (int (*)(const void*,const void*))
                         Node_compareString);
...
{
   assert(oNParent != NULL);
   return Node_getNumDirChildren(oNParent) +
      Node_getNumFileChildren(oNParent);
}

int  Node_getChild(boolean childIsDir, Node_T oNParent,
                   size_t ulChildID, Node_T *poNResult)
{   
   assert(oNParent != NULL);
   assert(poNResult != NULL);

   /* ulChildID is the index into oNParent->oDDirChildren or 
      oNParent->oDFileChildren*/
      if (childIsDir) {
         if (ulChildID >= Node_getNumDirChildren(oNParent)) {
            ...
            *poNResult = DynArray_get(oNParent->oDDirChildren,
                                      ulChildID);
            return SUCCESS;
         }
      }
         ...
         if (ulChildID >= Node_getNumFileChildren(oNParent)) {
            ...
            *poNResult = DynArray_get(oNParent->oDFileChildren,
                                      ulChildID);
            return SUCCESS;
         }
      }
